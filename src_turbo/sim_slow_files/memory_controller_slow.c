...
dram_address_t * calc_dram_addr(long long int physical_address) {
  ...
  //DONE_TODO : INSERT NEW MAPPING FOR NORMAL LINES & COUNTERS/MACs  & MERKLE TREE
...
int issue_request_command(request_t * request) 
{
	...
	switch(cmd)
	{
		case ACT_CMD :
			...
			memory_reqhit_inst[request->thread_id]++;
			memory_reqhit[request->thread_id]++;
			...
			if(request->operation_type == READ){
              ...
              //Memory logger for read activates
				memory_rdhit_inst[request->thread_id]++;
				memory_rdhit[request->thread_id]++;
				...
				memory_wrhit_inst[request->thread_id]++;
				memory_wrhit[request->thread_id]++;
		...
		case COL_READ_CMD :
            ...
            //DONE_TODO: Ensure Blocking Metadata Read Ops are indeed on the critical path.
            //Added by Gururaj

            // Check if current request is a critical request (source or dependency), or not
            if(request->critical){ //Either Critical Read of $line or Critical read of dependancy metadata
              ROB[request->thread_id].critical_mem_ops[request->instruction_id]--;
              lat_stat_process(lat_sim_stat, &(ROB[request->thread_id].read_lat_stat[request->instruction_id]), request->type,
                               CYCLE_VAL_CORE[request->thread_id], CYCLE_VAL_CORE[request->thread_id]+ T_CAS + T_DATA_TRANS,
                               ROB[request->thread_id].critical_mem_ops[request -> instruction_id], mem_org);
              
              
              if(ROB[request->thread_id].critical_mem_ops[request -> instruction_id] == 0){ //Last critical request finished
                // update the ROB with the completion time
                ROB[request->thread_id].comptime[request->instruction_id]=CYCLE_VAL_CORE[request->thread_id]+PIPELINEDEPTH+(((T_CAS + T_DATA_TRANS)*100)/core_mod[request->thread_id]);
              }
            }
            ...
            memory_rd_inst[request->thread_id]++;
            memory_rd[request->thread_id]++;
            memory_req_inst[request->thread_id]++;
            memory_req[request->thread_id]++;
            ...
            memory_rdqueue_inst[request->thread_id] = request->latency + memory_rdqueue_inst[request->thread_id];
            memory_rdqueue[request->thread_id] = request->latency + memory_rdqueue[request->thread_id];
            memory_reqqueue_inst[request->thread_id] = request->latency + memory_reqqueue_inst[request->thread_id];
            memory_reqqueue[request->thread_id] = request->latency + memory_reqqueue[request->thread_id];	

            memory_rdqueue_ch_inst[channel] = request->latency + memory_rdqueue_ch_inst[channel];
            memory_rdqueue_ch[channel] = request->latency + memory_rdqueue_ch[channel];
            memory_reqqueue_ch_inst[channel] = request->latency + memory_reqqueue_ch_inst[channel];
            memory_reqqueue_ch[channel] = request->latency + memory_reqqueue_ch[channel];	

            memory_rdqueue_total_inst = request->latency + memory_rdqueue_total_inst;
            memory_rdqueue_total = request->latency + memory_rdqueue_total;
            memory_reqqueue_total_inst = request->latency + memory_reqqueue_total_inst;
            memory_reqqueue_total = request->latency + memory_reqqueue_total;
		...
		case COL_WRITE_CMD :
			...
			//Update the memory loggers
			memory_wr_inst[request->thread_id]++;
			memory_wr[request->thread_id]++;
			memory_req_inst[request->thread_id]++;
			memory_req[request->thread_id]++;
			...
			memory_wrqueue_inst[request->thread_id] = request->latency + memory_wrqueue_inst[request->thread_id];
			memory_wrqueue[request->thread_id] = request->latency + memory_wrqueue[request->thread_id];
			memory_reqqueue_inst[request->thread_id] = request->latency + memory_reqqueue_inst[request->thread_id];
			memory_reqqueue[request->thread_id] = request->latency + memory_reqqueue[request->thread_id];	

			memory_wrqueue_ch_inst[channel] = request->latency + memory_wrqueue_ch_inst[channel];
			memory_wrqueue_ch[channel] = request->latency + memory_wrqueue_ch[channel];
			memory_reqqueue_ch_inst[channel] = request->latency + memory_reqqueue_ch_inst[channel];
			memory_reqqueue_ch[channel] = request->latency + memory_reqqueue_ch[channel];	

			memory_wrqueue_total_inst = request->latency + memory_wrqueue_total_inst;
			memory_wrqueue_total = request->latency + memory_wrqueue_total;
			memory_reqqueue_total_inst = request->latency + memory_reqqueue_total_inst;
			memory_reqqueue_total = request->latency + memory_reqqueue_total;
